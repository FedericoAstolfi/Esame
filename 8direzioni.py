...
if SWITCH_VELENO:
    ...
    LEN_GENOMA = 2**8
    POSSIBILITA = [format(i, "08b") for i in range(0,2**8)]
...
class Creature():
    ...
    def __init__(self, dizionario = None, x=None, y=None, energia = ENERGIA  ):
        ...
        if dizionario: #gli ho dato qualche dizionario
            ...
            self.mosse = {i : random.randint(0, 7) for i in POSSIBILITA} #0 è EST, 1 NE, 2 N, 3NO, 4 O, 5 SO, 6 S, 7 SE
...
def movimento(creatura, ambiente):
    ...
    if creatura.energia > 0: #controllo che abbia energia per muoversi
        ...
        chiave_list = [ambiente[x][(y+1)%NGRIGLIA], ambiente[(x-1)%NGRIGLIA][(y+1)%NGRIGLIA],\
                        ambiente[(x-1)%NGRIGLIA][y], ambiente[(x-1)%NGRIGLIA][(y-1)%NGRIGLIA],
                        ambiente[x][(y-1)%NGRIGLIA],ambiente[(x+1)%NGRIGLIA][(y-1)%NGRIGLIA],
                        ambiente[(x+1)%NGRIGLIA][y], ambiente[(x+1)%NGRIGLIA][(y+1)%NGRIGLIA]]
...
def greed(dict):
    ...
    for k in chiavi: #scorro sulle chiavi
        for i in range(0,8): #scorro sugli 8 bit
...
if __name__ == '__main__':
    ...
    main(npop= 114, mut_prob=0.1, ngen=300, cut_crss= CUT_CRSS, ntorte= 300, grafici= False, scritte = True)

#risultati buoni con griglia da 20, npop 114 e 297 torte (ottenuti più o meno trasportando le proporzioni di 30 80 in griglia da 10)

# 16 geni 5 mosse ---> 256 geni 120 mosse (240 energia)