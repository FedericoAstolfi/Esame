...
'''0101 significa: dx niente, su torta, sx niente, giù torta
   ...
   si legge da sinistra verso destra e la posizione 0 indica
   ciò che c'è a dx, la posizione 1 ciò che c'è in alto, la
   2 ciò che c'è a sx, la 3 ciò che c'è giù '''
...
class Creature():
    ...
    def __init__(self, dizionario = None, x=None, y=None, energia = ENERGIA  ):
        ...
        if dizionario:
            ...
            '''"0120" : 3 significa, ad esempio, che se non ho nulla a dx e in basso, ho una torta su e un veleno a sx,
            allora il 3 mi dice di andare in basso, dove non c'è niente'''
    ...
    def mate(self, other, mut_prob):
        ...
        a = round(((self.energia + other.energia)*0.5)+0.1)  
        # vista la natura del problema aggiungere 0.1 mi assicura di arrotondare per eccesso
...
def movimento(creatura, ambiente):
    ...
    creatura ed eventualmente rimuove una torta'''
        ...
        for i in range(npop):
            ...
            if parent1.mosse != parent2.mosse:    #se ho preso due parenti diversi li tengo e accoppio
                ...
                while parent1.mosse == parent2.mosse and parent1.x == parent2.x and parent1.y == parent2.y and contatore <=100:
                    parent1 = roulette_sampling(parents, fit)
                    contatore += 1
                    '''questo while viene ripetuto molte volte in alcune situazioni
                       (per esempio se rimangono solo due creature, una con energia
                       1 e l'altra con energia 0.0001). Dunque compare un ulteriore
                       controllo che dopo 100 cicli arresta il while e fa accoppiare
                       il genitore con se stesso'''
...
def fear(dict):
    '''dato un dizionario conta in quante configurazioni viene evitato il veleno, quando c'è'''
    count = 0
    chiavi = list(dict.keys())
    for k in chiavi:
        if '2' in k:    
