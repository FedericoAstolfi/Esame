...
class Creature():
    ...
    def mate(self, other, mut_prob):
        ...
        '''qua non faccio la media ma do sempre ENERGIA'''
        c.energia = ENERGIA  
        return c    
...
def get_offsprings(parents, npop, mut_prob, scritte): #lista di creature e prob di mutazione
    ...
    if fit.count(0) == npop -1:
        ...
        '''però in questa versione beta del codice, non voglio rimettergli la stessa energia, ma gliela alzo a 10
        PENSA A QUANTO CI0'POSSA RAPPRESENTARE UN PROBLEMA'''
...
def main(npop, mut_prob, ngen, cut_crss = CUT_CRSS, ntorte = 60, grafici = True, scritte = True, nvel = 5):
